namespace :reminder_message_task do
  desc "Rake task to send reminder sms to the user prior to the events"
  ...
  task :send => :environment do
    ...
    if one_day_prior_user_all_events.present?
      one_day_prior_user_all_events.each do |event|
        if Date.today+ 1.day == event.date
          begin
            UserReminder.send_reminder_message(event).deliver
          rescue Exception => e
            puts e.message
          end
        end

        puts "#{Time.now} - Success! 1 day prior SMS reminder for user: #{event.user.email} for event: #{event.program}."
      end
    end
    ...
    if three_day_prior_user_all_events.present?
      three_day_prior_user_all_events.each do |event|
        ...
        puts "#{Time.now} - Success! 3 day prior SMS reminder for user: #{event.user.email} for event: #{event.program}."
      end
    end
    ...
    if seven_day_prior_user_all_events.present?
      seven_day_prior_user_all_events.each do |event|
        ...
        puts "#{Time.now} - Success! 7 day prior SMS reminder for user: #{event.user.email} for event: #{event.program}."
      end
    end
  end
